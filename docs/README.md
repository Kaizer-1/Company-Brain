...
| [decisions/0034-in-memory-metrics-for-demo-scale.md](decisions/0034-in-memory-metrics-for-demo-scale.md) | Phase 5B: in-memory metrics registry + JSON `/api/metrics` over prometheus_client/OpenTelemetry; why a dashboard is over-instrumentation at one process and tens of ingestions; process-local/volatile honesty; the production scaling path (per-worker registries → Prometheus/OTel) with the `record_*` call sites unchanged |
| [decisions/0035-parallel-resolution-adjudication.md](decisions/0035-parallel-resolution-adjudication.md) | Phase 5B: parallelise Tier-2 resolution adjudication with `Semaphore(5)`, Tier-1-first, serial writes; the metrics corrected the premise (the 15s tail was embedding cold-start, not adjudication); measured 4.0× speedup under forced fan-out (45.7s→11.3s on 16 calls); why bounded not unbounded, why serial writes not per-task sessions |
...
| [design/observability.md](design/observability.md) | Phase-5B observability: the in-memory metrics layer (counters/histograms/adjudication counters), the two read surfaces (`/api/metrics` distributions vs the `ingestion_runs` audit ledger), why in-memory at demo scale, the production scaling path (Prometheus/OTel/Redis), the three-item scope rationale, and how the measurement found the 15s tail was cold-start not adjudication |
...
| [eval/phase-5b-observability-results.md](eval/phase-5b-observability-results.md) | Phase-5B results: 11-case eval still 100% pass with the parallel resolver (mean 6.1s); the metrics-driven finding that the 5A 15s `doc-new-person` tail was the embedding cold-start (zero Tier-2 calls), not adjudication; the controlled semaphore A/B isolating the parallelisation (4.0× under forced fan-out, 45.7s→11.3s); the `/api/metrics` snapshot; an ASCII of the ingestion-runs audit tab; honest Discussion that the measurement was worth more than the optimisation |
...
| [interview-prep/phase-5b-readiness.md](interview-prep/phase-5b-readiness.md) | 8 Q&A pairs: in-memory metrics vs Prometheus/OTel, why semaphore(5) not unbounded, the Tier-1-first ordering, the measurement-driven narrative (metrics corrected the premise), metrics-vs-audit surfaces, cursor vs offset pagination, parallel-correctness (same final merge set), and the deferred SSE/per-node-locking scope calls |
